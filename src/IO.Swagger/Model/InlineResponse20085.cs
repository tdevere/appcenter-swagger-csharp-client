/* 
 * App Center Client
 *
 * Microsoft Visual Studio App Center API
 *
 * OpenAPI spec version: v0.1
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */
using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using SwaggerDateConverter = IO.Swagger.Client.SwaggerDateConverter;

namespace IO.Swagger.Model
{
    /// <summary>
    /// Event properties during the time range.
    /// </summary>
    [DataContract]
        public partial class InlineResponse20085 :  IEquatable<InlineResponse20085>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="InlineResponse20085" /> class.
        /// </summary>
        /// <param name="eventProperties">eventProperties.</param>
        public InlineResponse20085(List<string> eventProperties = default(List<string>))
        {
            this.EventProperties = eventProperties;
        }
        
        /// <summary>
        /// Gets or Sets EventProperties
        /// </summary>
        [DataMember(Name="event_properties", EmitDefaultValue=false)]
        public List<string> EventProperties { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class InlineResponse20085 {\n");
            sb.Append("  EventProperties: ").Append(EventProperties).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as InlineResponse20085);
        }

        /// <summary>
        /// Returns true if InlineResponse20085 instances are equal
        /// </summary>
        /// <param name="input">Instance of InlineResponse20085 to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(InlineResponse20085 input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.EventProperties == input.EventProperties ||
                    this.EventProperties != null &&
                    input.EventProperties != null &&
                    this.EventProperties.SequenceEqual(input.EventProperties)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.EventProperties != null)
                    hashCode = hashCode * 59 + this.EventProperties.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }
}
